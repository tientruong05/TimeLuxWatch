    ...
    <div class="section">
      <div class="container" v-if="product">
          ...
          <div class="col-md-6">
            ...
            <p class="product-price">
              <span v-if="product.discountPercentage">
                <span class="discount-price">{{ formatCurrency(product.discountedPrice) }}</span>
            ...
            <div class="product-info">
              <p><strong>Số lượng còn:</strong> <span>{{ product.qty }}</span></p>
              <p><strong>Mô tả:</strong> <span>{{ product.description }}</span></p>
              <p>
                <strong>Trạng thái:</strong>
                <span :class="product.qty > 0 ? 'stock-status-in' : 'stock-status-out'">
            ...
            <div class="d-flex gap-3 mt-4">
              <div class="input-group mb-3" style="max-width: 200px">
                <button class="btn btn-outline-secondary" type="button" @click="decrementQuantity" :disabled="product.qty <= 0">
                  -
                </button>
                <input
                  type="number"
                ...
                />
                <button class="btn btn-outline-secondary" type="button" @click="incrementQuantity" :disabled="product.qty <= 0">
                  +
                </button>
              </div>
            ...
            <div class="d-flex gap-3">
              <button class="btn btn-buy" @click="buyNow" :disabled="product.qty <= 0">Mua ngay</button>
              <button class="btn btn-cart" @click="addToCart" :disabled="product.qty <= 0">Thêm vào giỏ</button>
            </div>
  
            <div class="delivery-info">
              <p class="mb-2"><strong>Giao hàng:</strong> Freeship Hà Nội & HCM</p>
        ...
        <div class="related-products" v-if="relatedProducts.length">
          ...
          <div class="row">
            <div class="col-md-2" v-for="related in relatedProducts" :key="related.id">
              <div class="related-product-item">
                <a :href="`/java5/asm/products/detail/${related.id}`">
                  <img :src="`/photos/${related.image}`" :alt="related.name" class="img-fluid" />
        ...
        fetch(`/java5/asm/cart/add/${this.product.id}`, {
          ...
          .then((response) => {
            if (response.status === 401) {
              window.location.href = `/java5/asm/login?redirectUrl=${encodeURIComponent(window.location.pathname)}`;
